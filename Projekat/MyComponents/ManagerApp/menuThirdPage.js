...
import { StyleSheet, Text, View,TextInput,Button,TouchableOpacity ,CheckBox,ImageBackground,Dimensions,Picker,Alert} from 'react-native';
...
export default class menuThirdPageScreen extends React.Component {//desert
  ...
  createDesert=()=>{
    ...
    .then((response)=>{
     if(response==true)
     Alert.alert("Obaveštenje","Kreirali ste desert.");
     else
     {
...
<View style={{display:'flex',flexDirection:'column',flex:10,}}>
    ...
    <View style={{flex:10,display:'flex',flexDirection:'row'}}>
        ...
        <LinearGradient start={{x: 0, y: 0}} end={{x:0 , y: 1}} colors={['white','#F1F1F1']} style={{flex:1,flex:8.5,display:'flex',flexDirection:'row',borderRadius:20,borderColor:'grey',borderWidth:1}}>
          ...
          <View style={{flex:4,display:'flex',flexDirection:'column'}}>
             ...
             <TextInput placeholder="Ime deserta" placeholderTextColor='#fbb0a9' onChangeText={this.onTextChangeName} style={{flex:1.5,borderBottomColor:'black',
  ...
  mode='dropdown'
  style={{flex:1,color:'#fbb0a9',borderColor:'black',borderWidth:2,fontSize:15}}
  ...
  <Picker.Item label="Platinum" value="platinum" />
            ...
            <View style={{flex:1.5,display:'flex',flexDirection:'row',alignContent:'center',justifyContent:'center'}}>
              ...
              <TouchableOpacity onPress={this.createDesert} style={{display:'flex',alignItems:'center',justifyContent:'center',borderRadius:25,backgroundColor:'#fbb0a9',flex:1}}><Text style={{fontSize:20}}> Kreiraj desert</Text></TouchableOpacity>
              </View>
         <View style={{flex:2}}></View>
