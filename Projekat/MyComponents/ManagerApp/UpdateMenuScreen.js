...
export default class UpdateMenuScreen extends React.Component {
...
this.returnStarters();
    ...
    pickerChange(index){
      this.state.predjela.map( (v,i)=>{
       if( index === i ){
         this.setState({
         toChange: this.state.predjela[index].name
        })
       }
      })
     }
    ...
    updateStarter=()=>{
      ...
      formData.append("updateStarterManager",1);
   ...
   returnStarters=()=>{
    const formData=new FormData();
    formData.append("returnStartersToUpdate",1);
    const fetchData={
      method:"post",
      body:formData
    };
    ...
    .then((response)=>response.json())
    .then((response)=>{
      this.setState({predjela:response,toChange:response[0]});
    })
    .catch((error)=>{alert(error);});
   }
...
render() {
    return (
      ...
      <View style={{display:'flex',flexDirection:'column',flex:10}}>
          <View style={{flex:2.2,alignContent:'stretch',justifyContent:'center'}}><Text style={{fontFamily:'cursive',fontSize:33,color:'black'}}>Izmena predjela</Text></View>
          <View style={{flex:10,display:'flex',flexDirection:'row'}}>
              <View style={{flex:0.5}}></View>
              <LinearGradient start={{x: 0, y: 0}} end={{x:0 , y: 1}} colors={['white','#F1F1F1']} style={{flex:1,flex:8.5,display:'flex',flexDirection:'row',borderRadius:20,borderWidth:1,borderRadius:20,borderColor:'white',shadowOffset:{width:2,height:4},shadowOpacity:0.8,shadowRadius:2,elevation:5,shadowColor:'#000'}}>
...
/>
  ...
  <Picker.Item label="Platinum" value="platinum" />
                  ...
                  <View style={{flex:2}}></View>
           <View style={{flex:1.5}}><TouchableOpacity style={{flex:1,flexDirection:"column",borderRadius:30, alignItems:'center',justifyContent:'center',backgroundColor:'#fbb0a9'}} onPress={this.updateStarter}><Text style={{color:'white'}}>Izmeni predjelo</Text></TouchableOpacity></View>
